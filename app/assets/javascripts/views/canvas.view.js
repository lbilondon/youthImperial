define([
	...
	function($, _, Backbone, CanvasTemplate, UserModel) {
		...
		return Backbone.View.extend({
			...
			initialize : function() {
				_.bindAll(this, 'render', 'getImageData', 'manipulateImage');
				this.render();
			},
				...
				if (this.canvas !== undefined) {
					...
					model.fetch({
						success: function (rModel) {
							...
							that.getImageData(img1_src, img2_src, that.manipulateImage);
			...
			getImageData: function(image1Src, image2Src, callback) {
				var height = 615,
					width = 615,
					img1Loaded = false,
					img2Loaded = false,
			...
			manipulateImage: function(image1Data, image2Data) {
				...
				while (pixels--) {
					tmp.data[pixels] = (image1Data.data[pixels] * 0.5) + (image2Data.data[pixels] * 0.5);
				}
				...
				var colPos1 = Math.round((this.canvas.width/100)*45);
				var colPos2 = Math.round((this.canvas.width/100)*75);
				...
				this.$el.append('<a href="' + finalImageUrl + '">download image</a>');
			}
		});
	}
);
